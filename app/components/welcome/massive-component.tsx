  ...
  return (
    <div className='space-y-4'>
      <div className='inline-flex items-center gap-3 rounded-full border border-slate-200 bg-white px-3 py-2 shadow-sm'>
        <AppIcon size='small' />
        <span className='text-xs font-semibold uppercase tracking-[0.22em] text-slate-500'>Conversation Setup</span>
      </div>
      <div>
        <div className='text-3xl font-semibold text-slate-900'>{t('app.common.welcome')} {siteInfo.title}</div>
        <p className='mt-3 max-w-2xl text-sm leading-6 text-slate-500'>{siteInfo.description || 'Fill in the setup below and start chatting with your Dify assistant.'}</p>
      </div>
    </div>
  )
}
...
export const PromptTemplate: FC<{ html: string }> = ({ html }) => {
  return (
    <div
      className='box-border text-sm leading-6 text-slate-700'
...
export const ChatBtn: FC<{ onClick: () => void, className?: string }> = ({
  ...
  return (
    <Button
      type='primary'
      className={cn(className, `space-x-2 flex items-center rounded-full px-5 ${s.customBtn}`)}
      onClick={onClick}
    >
...
export const EditBtn = ({ className, onClick }: { className?: string, onClick: () => void }) => {
  ...
  return (
    <div
      className={cn('flex items-center space-x-1 rounded-full px-3 py-1.5 cursor-pointer text-slate-600 transition hover:bg-slate-50', className)}
      onClick={onClick}
    >
...
export const FootLogo = () => (
  <div className='flex items-center gap-2'>
    <Image src='/brand-icon.svg' alt='Brand logo' width={20} height={20} />
    <span className='text-slate-500'>Dify</span>
  </div>
)
